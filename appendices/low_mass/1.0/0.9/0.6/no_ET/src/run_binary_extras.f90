module run_binary_extras
   ...
   logical :: too_slow = .false., convective_et = .true.
...
contains
   ...
   integer function extras_binary_start_step(binary_id, ierr)
      ...
      if (b% lxtra(ilx_pre_ms)) then
         b% fr = -1d0
         if (b% s1% center_h1 < 0.7145d0 .and. b% s2% center_h1 > 0.7145d0) then
            b% s1% dxdt_nuc_factor = 0d0  ! let secondary catch up to zams
         end if
         return
      else
         b% fr = inlist_fr
...
!      ! smooth composition of outer 0.01 Msuns of accretor
   ...
   integer function extras_binary_check_model(binary_id)
      ...
      if (b% lxtra(ilx_pre_ms) .and. b% s1% center_h1 < 0.7135d0 .and. b% s2% center_h1 < 0.7135d0) then
   ...
   integer function extras_binary_finish_step(binary_id)
      ...
      if (b% lxtra(ilx_pre_ms) .and. &
            ...
            b% s1% star_age > pre_ms_time .and. &
            b% s1% center_h1 < 0.7145d0 .and. b% s2% center_h1 < 0.7145d0) then
         ...
         b% s1% dxdt_nuc_factor = 1d0  ! activate nuclear burning again.
         ...
         if (b% rl_relative_gap(b% d_i) > 0) then
            ...
            (abs(log10(abs(b% s1% L_nuc_burn_total * Lsun / b% s1% L(1)))) > 0.005 .or. &
                  ...
                  b% s1% star_age < pre_ms_time .or. &
                  b% s1% center_h1 > 0.7145d0 .or. b% s2% center_h1 > 0.7145d0)) then
   ...
   subroutine calculate_transfer(binary_id, loc_1, loc_2, trans_1, trans_2, ierr)
      ...
      integer, intent(out) :: ierr
      type (binary_info), pointer :: b
      real(dp) :: lq, l1, l2, s1, s2, new_l1_rad, new_l2_rad, new_l1_conv, new_l2_conv, a, s_g1, s_g2
      ...
      s1 = eval_area(lq, b% s1% r(cell_1) / b% rl(1), ierr) * a * a
      s2 = eval_area(-lq, b% s2% r(cell_2) / b% rl(2), ierr) * a * a
      new_l1_conv = (l1 + l2) * s1 / (s1 + s2)
      new_l2_conv = (l1 + l2) * s2 / (s1 + s2)
      ...
      new_l1_rad = (l1 + l2) * s_g1 / (s_g1 + s_g2)
      new_l2_rad = (l1 + l2) * s_g2 / (s_g1 + s_g2)

   !      write(*, *) new_l1_conv - l1, new_l1_rad - l1
      if (convective_et) then
         trans_1 = new_l1_conv - l1
         trans_2 = new_l2_conv - l2
      else
         trans_1 = new_l1_rad - l1
         trans_2 = new_l2_rad - l2
      end if
   !   write(*, *) "old luminosities = ", l1, l2, l1+l2
   !   write(*, *) "new luminosities = ", new_l1_rad, new_l2_rad, new_l1_rad + new_l2_rad
   !   write(*, *) "lum. to transfer for shellularity = ",&
   !         trans_1, trans_2
   ...
   subroutine my_check_jdot_mb_conditions(b, s, apply_jdot_mb, qconv_env)
      type (binary_info), pointer :: b
      type (star_info), pointer :: s
      ...
      ! calculate how much of inner layers are convective
      qconv_core = 0d0
      do k = s% nz, 1, -1
   ...
   end subroutine my_check_jdot_mb_conditions

   subroutine my_jdot_mb(binary_id, ierr)
      integer, intent(in) :: binary_id
      integer, intent(out) :: ierr
      type (binary_info), pointer :: b
      type (star_info), pointer :: s
      real(dp) :: logm, turnover_factor, p_rot_days, p_crit_days, qconv_env, jdot_scale, non_sat_mb, sat_mb
      integer :: i, k, id
      ...
      b% jdot_mb = 0
      if (b% lxtra(ilx_pre_ms)) return
   !   write(*, *) "MB calculations"
      do i = 1, 2
         id = b% star_ids(i)
         call star_ptr(id, s, ierr)
         call my_check_jdot_mb_conditions(b, s, apply_jdot_mb, qconv_env)
         if (.not. apply_jdot_mb) cycle
         ...
         logm = log10(s% m(1) / Msun)
         turnover_factor = exp10(-1.49 * logm - 0.54 * logm * logm)  ! eq 11 from Wright et al 2011
         p_crit_days = 2.8*turnover_factor
         p_rot_days = 2*pi / (s% omega(1) * secday)
         sat_mb = -jdot_scale * 1.04d35 * (1d0 / p_rot_days) * &
            pow(p_crit_days, -2d0) * sqrt(s% r(1) / Rsun) * pow(s% m(1) / Msun, -0.5d0)
         non_sat_mb = b% jdot_mb - jdot_scale * 1.04d35 * pow(p_rot_days, -3d0) * &
            sqrt(s% r(1) / Rsun) * pow(s% m(1) / Msun, -0.5d0)

   !      write(*, *) i, apply_jdot_mb, sat_mb, old_mb, p_crit_days
         if (apply_jdot_mb .and. p_rot_days <= p_crit_days) then
            b% jdot_mb = b% jdot_mb + sat_mb
         elseif (apply_jdot_mb) then
            b% jdot_mb = b% jdot_mb + non_sat_mb
         end if
      end do

   !      write(*, *) 'total, old mb', b% jdot_mb, old_mb

   end subroutine my_jdot_mb
