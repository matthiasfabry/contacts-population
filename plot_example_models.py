...
# %% appendix models
def plot_whole(prim_ET, sec_ET, prim_NO_ET, sec_NO_ET, prim_whole, sec_whole, name='example1', show_wind=False):
    fig = plt.figure()
    gs = gridspec.GridSpec(3, 1, hspace=0.0, height_ratios=(3, 2, 2))
    axs = np.empty((3, 1), dtype=object)
    myr = 1e6
    ...
    if show_wind:
        md.plot(prim_NO_ET, 'star_age', 'lg_wind_mdot_1', 'b--', xscale=myr, zorder=-2)
    ...
    legend_elements = [lines.Line2D([0], [0], color='orange', label='whole envelope'),
                       ...
                       lines.Line2D([0], [0], color='blue', label='no ET')]
    if show_wind:
        md.plot(prim_whole, 'star_age', 'lg_wind_mdot_1', 'orange--', xscale=myr)
        legend_elements.extend([lines.Line2D([0], [0], color='gray', label=r'$\dot{M}_{\rm trans}$'),
                                lines.Line2D([0], [0], color='gray', linestyle='--', label=r'$\dot{M}_{\rm wind}$')])
    plt.ylabel(r'$\log \frac{\dot{M}}{M_\odot / {\rm yr}}$')
    plt.legend(handles=legend_elements, loc='upper right', fontsize=5)
    ...
    # plt.savefig('pngs/'+name)  # plt.close()


case5 = glob.glob('../small_ET_population/full_grid/30.0/1.52/0.9/')[0]
...
sec_whole.data['star_age'] -= age
fig = plt.figure()
gs = gridspec.GridSpec(3, 2, hspace=0.0, height_ratios=(2, 2, 2), width_ratios=(2, 5), wspace=0.03)
...
plt.gca().yaxis.set_tick_params(labelright=True, labelleft=False)
md.plot_q(prim, 'r', xscale=gyr, label='$q$')
...
plt.plot(prim.get('star_age')[ix:] / gyr, running_q, 'b', label=r'$\tilde{q}_{\rm KH}$')
plt.ylabel('')
plt.xlabel(r'age (Gyr)')
